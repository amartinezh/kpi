...
public class Manufacture1DaoImpl implements Manufacture1Dao {
	...
	public List<reporteManufacture1> listSales(session ses, String table) {
		...
		for (CfgManufacture1 cfg : indicadores) {
			...
			if (list.get(1).getMvedes() != null){
				valor.add(new reporteManufacture1(list.get(1).getMvedes(), cfg.getIndicador(), 
		...
		for (CfgManufacture1 cfg : indicadores) {
			...
			*/
			...
			if (list.get(0).getMvedes() != null){
				valor.add(new reporteManufacture1(list.get(0).getMvedes(), cfg.getIndicador(), 
						cfg.getUnidad(), "Actual", "Budget",
						anioAntReal,
						anioAntPres,
						promMvevalRealAnoActual, 
	...
	operacion[1]="("+ses.getOp3r()+"(k.mvevpe))";
		...
		while (x<result.size()){
			...
			if (!ses.getCampo_llave().equals("mvecli")){
				...
				.createQuery(
						"Select k."+ses.getCampo_llave()+", k."+ses.getCampo_descripcion()+", k.mveano as mveano, "+operacion[0]+" as mveval, "+operacion[1]+" as mvevpe"
								+ " From "+table+" as k where k."+ses.getCampo_llave()+" = '"
			...
			valor.add(new reporteManufacture1(result.get(x)[0].toString(), result.get(x)[1].toString(), 
					"", "Actual", "Budget",
					anioAntReal, 
					anioAntPres,
					promedio, 
	...
	// //////////////////////////////////////////////////////////////////////////////////
    	...
    	operacion[1]="("+ses.getOp3r()+"(k.mvevpe))";
			...
			for (Object[] r : result) {
				...
				//if (list.get(0).getMvedes() != null){
					valor.add(new reporteManufacture1(r[0].toString(), r[1].toString(), 
							"", "Actual", "Budget",
							anioAntReal,
							anioAntPres,
							promMvevalRealAnoActual, 
    	...
    	operacion[1]="("+ses.getOp3r()+"(k.mvevpe))";
			...
			for(java.util.Map.Entry<String, List<BigDecimal>> entry : tm.entrySet()) {
				  ...
				  valor.add(new reporteManufacture1(key, valor_row_rotulos.get(key), 
							"", "Actual", "Budget",
							value.get(1),
							anioAntPres,
							value.get(2), 
